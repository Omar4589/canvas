...
| [FBTIME_INTEGRATION.md](FBTIME_INTEGRATION.md) | The opt-in FbTime connection: measured clock hours replacing the knock-span estimate in doors-per-hour, the measured/estimated labels and the never-blend rule, key storage, the sync jobs, and canvasser mapping. |
...
| [EXPORTS.md](EXPORTS.md) | The Export Center: background CSV/ZIP exports of canvassing activity, doors by round, survey results, the voter file (reconstructed), saved-search subsets, notes, and the full backup — DNC-excluded, TTL'd artifacts, and the read-only wind-down carve-out. |
| [WALK_PACKETS.md](WALK_PACKETS.md) | The Print studio: paper walk packets for volunteers without the app — the two layouts, walk order on paper, splitting big books into ~N-door packets at print time, the cover map, suppression, the 1,200-door cap, and the print-only contract (nothing on paper ever syncs back). |
...
| [ADMIN_APP.md](ADMIN_APP.md) | The mobile admin app: the bottom-tab nav (Overview · Timeline · Map · Books · More), the "More" hub and its report screens (GPS audit, Notes, Exports, Overlaps, Duplicate surveys), and the Books screen for assigning turf/books to canvassers (by book / by canvasser, bulk). Super admins share these screens in-org. |
...
| [LOCK_SCREEN_AND_DIRECTIONS.md](LOCK_SCREEN_AND_DIRECTIONS.md) | The per-house **Directions** hand-off to the phone's maps app — **built 2026-09-05** on six mobile surfaces, address-based so a wrong pin cannot mislead, and shipped over the air. Plus the investigation behind it: a **Keep screen on** setting that was designed and **not** built, and the "lock screen feature" a canvasser reported in **RightInsight** — a teardown of its shipped app and live JS shows it has none, and the surface he saw is its Twilio dialer — with a seventeen-app census, the Android and iPhone routes if we ever want one anyway, and the privacy and release cost of each. |
