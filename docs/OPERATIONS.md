...
| `npm run repair:import-pins` | **Read-only and free** (dry run, cache-only). Finds doors an import pinned in the wrong place — out of state, far from their own street, sharing an exact pin with doors from *other streets* (vendor placeholder coordinates), or knocked from far away by 2+ canvassers — and re-checks each against the address. Narrow with `--campaign=<id>` / `--org=<slug>` |
