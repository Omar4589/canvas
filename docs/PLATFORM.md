...
- **The paging contract (batch 2).** Every super-admin list speaks the same shape, copied from
  ...
  never the truncated length. Lists that sort (`organizations`, `imports`, `users`) take an
  allowlisted `sort` — never raw user input into `.sort()` — and every users sort spec ends in
  `_id`, because a non-unique sort has no stable order across separate skip/limit windows and
  would let mobile's Load-more duplicate or drop a row. **Paging is opt-in**: a request with NO `skip`/`limit` keeps the exact
