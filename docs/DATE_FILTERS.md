...
| [pages/DashboardPage.jsx](../client/src/pages/DashboardPage.jsx) | Default **Today** in the **campaign** tz; range → `/campaign-rollup`, `/canvassers`, `/survey-results` (gated on the tz — §C), each also taking the page's walk-list (`effortId`) and crew (`coordinatorId`) filters. Coverage stays all-time (and crew-unfiltered) from `/overview`. An untouched **archived** campaign defaults to All time. |
