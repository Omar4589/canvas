...
- `POST …/turfs/restrict-bulk` / `unrestrict-bulk` (book-level) and `POST …/turfs/restrict-doors` /
  `unrestrict-doors` `{ householdIds[], passId? }` (single-home — the phone always sends exactly one id and
  no `scope`; the same route also serves the **web** maps' lassoed "Select doors" selection, up to 1,000 ids
  with an optional `scope`, see [PASSES_AND_TURF.md](PASSES_AND_TURF.md)) — one writer,
