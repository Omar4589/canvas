...
- **Needed** is `doors remaining ÷ calendar days left`, counting today. Calendar days, not
  canvassing days: days off are included.
- **Current pace** is the trailing **14 calendar days** of billable doors ÷ the days in that window,
  clipped to how long the campaign has been canvassing (its earliest `Pass.activatedAt`). Same
  calendar basis as *Needed*, so a crew that takes Sundays off is measured consistently on both
  sides and the two are directly comparable.
- **The verdict** — Ahead (≥1.05× needed) / On track (≥0.95×) / Behind — plus a projected finish
  date. It is **suppressed** below 5 days of canvassing, or when the trailing window holds no doors
  at all: a wrong "Behind" on a healthy campaign costs more trust than a missing verdict.
...
| `goal` | Door-goal block, or `null` when no goal is set. **ALL-TIME and campaign-wide even on a windowed/effort/crew-scoped request** — a goal is a whole-campaign contract number, so it deliberately ignores the row's own filters. Keys: `target`, `deadline`, `deadlineSource` (`goalDate`\|`electionDay`), `done`, `remaining`, `percent`, `daysLeft`, `requiredPerDay`, `requiredPerWeek`, `recentPerDay`, `paceWindowDays`, `verdict`, `projectedFinish`, `projectedDaysLate` | `goalProgressFor()` in [goalProgress.js](../server/src/services/reports/goalProgress.js) — `done` off `Campaign.stats` (live pipeline for unseeded docs), the trailing rate one `knocksPipeline` per distinct campaign timezone. **Zero queries when nothing in scope has a goal**, which keeps the rollup's `useStats` fast path fast | `/campaign-rollup`, `/admin/campaigns` | all-time (`done`) / trailing 14d (`recentPerDay`) |
...
| `goal.verdict` | `complete` \| `no_deadline` \| `past_due` \| `no_pace` \| `ahead` \| `on_track` \| `behind`. `no_pace` means "there is a goal but not enough canvassing to judge it" — the required rate is still populated, the projection is not | ordered ladder in `computeGoalPace()` (pure; [goalPace.test.js](../server/test/goalPace.test.js)) | `/campaign-rollup`, `/admin/campaigns` | — |
...
| [components/GoalProgressCard.jsx](../client/src/components/GoalProgressCard.jsx) | **Door goal** on campaign Home, above Activity: progress bar, doors left, Needed /day + /week, Current pace, verdict `Badge`, projection. Renders the `goal` block verbatim — no arithmetic. Its footer *"Campaign-wide, all time — not affected by the filters above"* is load-bearing: it is the ONLY number on that page that ignores the range/walk-list/crew pickers. Words + colors from [lib/goalPace.js](../client/src/lib/goalPace.js); compact `GoalCell`/`GoalBlock` for the campaigns table/cards are exported from `campaigns/CampaignCard.jsx`. |
