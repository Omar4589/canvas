...
> (a likely mistake) vs *different canvassers* (usually a legitimate revisit). Fix on **web** by opening
> the voter profile and deleting the extra response; on **mobile** the expanded card deletes in place
> (`DELETE /admin/voters/:voterId/surveys/:responseId`). That delete is **org-admin only** on both
> surfaces — `admin/voters.js` is `requireOrgRole('admin')` router-wide — even though the report itself
> is lead-readable, so a lead sees every duplicate and no delete. Deleting moves **`surveyCount` only**:
> the `survey_submitted` CanvassActivity row survives, so knock/door counts are untouched (pinned by
> `campaignStats.int.test.js`). The report is **paged** (`skip`/`limit`, default 25, max 100;
...
> **`?kind=sameCanvasserSameDay|differentCanvassers`** (the two flags, so an auditor can isolate the
> likely-mistake bucket).
...
| `GET /admin/reports/duplicate-surveys` | voters with >1 survey response; optional `?userId=` (groups containing that canvasser — matched **after** grouping, so the filter never changes what counts as a duplicate and the group still returns every response), `?kind=all\|sameCanvasserSameDay\|differentCanvassers`, `skip`/`limit` (default 25, max 100) | `{ duplicates[{ voterId, count, voter, household, responses[{ responseId, canvasser, submittedAt, day, passId, roundLabel }], sameCanvasserSameDay, differentCanvassers }], total, limit, skip, timeZone, tzAbbrev }`. **`total` is the true matching-group count** (it was the post-truncation page length before paging existed). Both flags are computed **in the aggregation** — `sameCanvasserSameDay` when a `(userId, local day)` key repeats (day bucketed in the anchor tz, so a 11:50 PM / 12:10 AM pair is two days), `differentCanvassers` when >1 distinct userId. Sorted `sameCanvasserSameDay` first, then `count` desc, `voterId` as the tiebreak so pages don't shuffle. | `submittedAt` |
...
| [campaign/[campaignId].jsx](../mobile/app/(app)/admin/campaign/[campaignId].jsx) | **Activity** tiles (Knocks, Surveys/Lit, Surveyed voters, Connection rate via `rateFromPct`) from rollup; **By pass** card (`/knocks-by-pass` over the same range — one row per walk list × round: knocks + "Conn N%"/"Lit N%"); **Coverage** (all-time) from overview; Top canvassers from `/canvassers`; "Timeline" quick-link. |
