...
| `DELETE .../turfs/:turfId` | Delete a **single DRAFT book** (surgical cleanup — e.g. one unwanted supplemental book): clears the household mirror and deletes the doc. Never touches `CanvassActivity` — a single-home desk mark made while the door sat in this draft keeps its row and counts under the door's next book. **409 `not-draft`** for accepted books (Discard is their only removal path). Brief `recutLock` hold so a supplemental/claim job can't race the delete. **No re-tessellation** — removing a book's doors only grows the neighbors' Voronoi entitlement, so every stored shape stays disjoint and containing. |
