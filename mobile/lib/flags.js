...
// The five flag reasons, in display order. `color` drives the map dots + badges.
...
// 250 m ≈ 820 ft). Mirrors client/src/lib/flags.js — keep the two in sync.
  ...
  switch (reason.type) {
    ...
    case 'weak_gps':
      ...
      if (d.accuracy != null && d.accuracy > FLAG_THRESHOLDS.GPS_ACCURACY_WARN_M)
        return `GPS ±${formatDistance(d.accuracy)}`;
...
// own earlier entry. Null unless a post-knock pin move exists. Mirrors client/src/lib/flags.js.
export function pinCorrectionText(entry) {
  const d = farDetailOf(entry);
  if (d?.pinCorrectedMeters == null) return null;
  const away = formatDistance(d.pinCorrectedMeters);
