...
// to the server and to client/src/lib/outcomeToggles.js by server/test/outcomeToggles.test.js,
// the same gate actionLabels uses for the label maps. Plain ESM, no React Native imports, so
// node can load it in that test.
//
// OUTCOME_HINTS is the admin Door Outcomes screen's one-line "how does this count?" copy — it
// describes the reporting semantics (docs/METRICS.md) so an admin knows what turning one off
// does NOT change: history keeps counting everywhere.
