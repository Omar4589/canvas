...
**What it is**: Internal door-to-door canvassing tool, originally built for the Scott Berger State Representative campaign (KY District 64) with one admin and ~3 canvassers. It has since grown into a multi-tenant platform — multiple organizations, each running their own campaigns with their own admins and canvassers, under a super-admin tier. Canvassers walk a list of geocoded households/voters imported from a CSV (~5,840 households / 8,668 voters for the original campaign), knock doors, record household status (unknocked / not_home / surveyed / wrong_address / lit_dropped), and capture survey responses tied to individual voters.
...
1. **CSV provides coordinates directly.** The CSV has `p_Latitude` / `p_Longitude` columns; the importer at `server/src/services/import/csvImporter.js` reads them and **requires** them — rows without valid coordinates are rejected. (An earlier Census + Mapbox geocoding subsystem was removed in commit `ce093f5`; if address-only lists ever need support it would be rebuilt as an optional step.)
...
- `server/src/models/{Organization,Membership,User,Campaign,CampaignAssignment,Household,Voter,SurveyTemplate,SurveyResponse,CanvassActivity,ImportJob}.js`
- `server/src/routes/{auth.js, admin/, mobile/, superAdmin/}`
- `server/src/utils/seedAdmin.js` — seeds admin user + (one-time) default survey
- `client/src/pages/{LoginPage, DashboardPage, ImportPage, MapPage, CampaignsPage, UsersPage, SurveysPage, SelectOrgPage, OrganizationsPage, SuperAdminHomePage, SuperAdminUsersPage, PrivacyPolicyPage}.jsx`
...
- `mobile/lib/{api.js, authState.js, auth.js, cache.js, config.js, location.js, offlineQueue.js}`
