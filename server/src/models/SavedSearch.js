...
const answerFilterSchema = new mongoose.Schema(
  {
    ...
    // The legacy-text half of the dual-read match (answerFilterClause). Undeclared
    // until the exclude branch landed, so strict mode silently dropped it on save.
    texts: { type: [String], default: undefined },
  },
  { _id: false }
);
...
// Shared by the include side and the NOT branch below. `combine` and `exclude` stay
// include-only: excludes always OR among themselves, and there are no nested NOTs.
const filterFields = {
  // Demographics
...
// The NOT branch: doors matching ANY of these are removed from the resolved set,
// unconditionally — `combine` never touches it (see resolveWalkList).
const excludeFilterSchema = new mongoose.Schema({ ...filterFields }, { _id: false });

const filterSchema = new mongoose.Schema(
  {
    ...filterFields,
    combine: { type: String, enum: ['and', 'or'], default: 'and' },
    exclude: { type: excludeFilterSchema, default: null },
  },
  { _id: false }
);
