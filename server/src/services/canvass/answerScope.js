 ...
 *     applies it exactly.
 */
export async function resolveAnswerScope({ campaign, template, scope, timestamp }) {
  const match = {
 ...
 */
export async function hydrateSurveyEvidence(rows, q) {
  const out = new Map();
  ...
  const responses = await SurveyResponse.find(
    { $or: or },
    { householdId: 1, passId: 1, userId: 1, voterId: 1, answers: 1 }
  ).lean();
  if (!responses.length) {
    ...
    for (const r of surveyed) out.set(String(r._id), { voters: 0, answers: 0, matchedVoters: 0, matched: [], otherNames: [] });
    return out;
  }
  ...
  const voters = await Voter.find({ _id: { $in: voterIds } }, { fullName: 1 }).lean();
  ...
  for (const resp of responses) {
    ...
    if (!byVisit.has(k)) byVisit.set(k, { voters: 0, answers: 0, matchedVoters: 0, matched: [], otherNames: [] });
    const b = byVisit.get(k);
    b.voters += 1;
    b.answers += (resp.answers || []).length;
    const name = nameById.get(String(resp.voterId)) || 'Unknown voter';
    ...
    } else if (b.otherNames.length < 5) {
      b.otherNames.push(name);
    }
  }
  ...
  for (const r of surveyed) {
    out.set(
      String(r._id),
      byVisit.get(doorKey(r)) || { voters: 0, answers: 0, matchedVoters: 0, matched: [], otherNames: [] }
    );
  }
  return out;
}
