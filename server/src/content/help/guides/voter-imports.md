...
**When many houses land on one dot.** Some vendors stamp a placeholder coordinate on addresses they couldn't place — the middle of a ZIP code, say — so unrelated houses from different streets pile onto one exact spot on the map. The preview now warns you: *"N doors sit on an exact map spot shared with doors from other streets."* Those doors still import and are still walkable; they're just drawn in the wrong place, and turf cutting's Remove apartments may mistake the pile for an apartment building and leave real houses out of your books. If you see that warning, ask your Doorline contact to run the pin repair before you cut turf.
