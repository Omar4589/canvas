...
## Marking one door restricted from the desk

A slate pin usually means a canvasser couldn't get to the door. You can put one there yourself — for a locked gate or a building nobody can get into — without marking the whole book. On the web map, click the door: its panel's **Restricted access** section offers **Mark restricted…** (one confirm, no note). On the phone, the admin Map tab's door sheet has a **Mark restricted** row above **Move pin**. Either way it's the same desk mark a whole-book restrict makes: canvassers see the door slate on their next sync, it never counts as a knock or in any rate, nobody gets credit for it, it's never billed, and the next round's cut can leave it out.
...
To work through **all** of them at once, use the [Pin Fixes page](page-pin-fixes): every approximate pin in the campaign grouped by street, with satellite imagery, a Google Maps shortcut, and a **Looks right — confirm** action for pins that check out (the ring goes out without a fake move, and the door reads *"Location confirmed"*). If the rings themselves clutter a busy all-time view, the Map page's **Layers** panel has an **"Approximate location rings"** checkbox to switch them off while you're on the page — they come back on next time you open the map.
