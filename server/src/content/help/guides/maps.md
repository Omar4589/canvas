...
To work through **all** of them at once, use the [Pin Fixes page](page-pin-fixes): every approximate pin in the campaign grouped by street, with satellite imagery, a Google Maps shortcut, and a **Looks right — confirm** action for pins that check out (the ring goes out without a fake move, and the door reads *"Location confirmed"*).
