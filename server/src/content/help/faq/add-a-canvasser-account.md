---
slug: add-a-canvasser-account
title: How do I create a canvasser account?
audience: lead
kind: faq
order: 28
sourceDoc: USERS.md
summary: Add them from a campaign roster (or the Users page) — they get an email invite to set their own password. If they already have an account, link it.
tags: canvasser, account, user, password, existing, link
---

The quickest way is **inline from a campaign's roster**: on the campaign's **Team** page, tap **+ New canvasser** and enter their name and email. That's it — they get an **invitation email** with a set-password link (valid 72 hours), they choose their own password, and they're in. This adds the account and drops them onto that campaign in one step. Both **team leads** and **admins** can do this.

The **temporary password field is optional**: leave it blank and the emailed invite is their way in. Type one only when the person can't get to their inbox — it can be simple and easy to read out loud, and they're required to replace it with their own strong password the first time they sign in. Emails never contain a password either way.

Admins can also add people org-wide from the **Users** page — same idea, and you can set the role (canvasser, team lead, or admin) there.

> Heads up: a temporary password is valid for **72 hours**. Have the new person sign in and set their own password within that window — if it lapses, just set a new temporary password for them.

**What if they already have a Door Line account?** If someone has canvassed before — even for a different organization — their email is already in the system, so you can't create a duplicate. When you enter an email that already exists, we switch on **Existing user** for you: confirm it, and they're linked to your organization and campaign. They keep the password they already use, so no new temporary password is issued. (If they're already in *your* organization, just add them from the campaign roster instead.)

Once the account exists, assign them a book so they can start knocking. See [Assigning canvassers](assigning-canvassers) and [User roles and your team](roles-and-team).
