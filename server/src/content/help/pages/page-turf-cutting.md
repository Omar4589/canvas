...
**Filtering the books.** The row of pills above the map filters the books — and their dots — two ways at once: by **assignment** (*Assigned / Unassigned*) and by **progress** (*Completed / In progress / Not started / Restricted*). Picks in the same family add together, and picks across the two families combine — so *Unassigned* + *Not started* + *In progress* shows unassigned books that still have work left, without pulling finished books back in. A book shows as **Restricted** when every door in it is restricted or no-soliciting this round: a book taken off the table isn't counted as *Completed*, so *Completed* means books your crew actually worked to the end. **Clear** puts everything back.

**Reading the numbers up top.** The strip above the map counts different things on purpose. **Books** and **Houses** are the size of the round. **Canvassers** is how many *people* hold at least one book, with the books nobody holds beside it ("78 books unassigned"). The *Assigned* pill to the right counts **books**, not people — so on a round where 8 people are carrying 14 books, the header reads 8, the pill reads 14, and neither is wrong. The **Crew load** pill at the bottom of the map puts it in one sentence: "8 canvassers · 14 books" — what your crew is actually carrying, not the size of the pass. Open it for each person's own books and doors.

**Draft books aren't counted as unassigned.** Everything about assignment and progress — the pills, the "N books unassigned" note, **Crew load** — covers your **accepted** books only. A draft can't be handed to anyone yet, so calling it unassigned would overstate the work left to hand out. **Books** still counts your whole pass and tells you how many are drafts, which is why the pills can add up to less than it (92 accepted + 5 draft = 97 books). Picking a pill also hides drafts from the list and map, since there's no status on them to filter by.

**If someone on the crew is deactivated.** Deactivating a person doesn't take their books away — that's on purpose, so their work and place in the round are kept. But it does mean a book can look covered when nobody who can still log in is holding it. The page now tells you: an amber note under the numbers counts how many of the crew are deactivated and how many books nobody on your roster can open, their initials show amber in the book list, and **Crew load** marks them. Nothing is re-counted behind your back — if you want those books back in the pool, **Unassign** them.

**Hiding restricted and no-soliciting homes.** Once status colors are on, restricted and no-soliciting homes draw as the darkest dots on the map, and a round with a lot of them can bury the live work. A **Restricted & no soliciting** checkbox in the **Layers** box (it appears whenever there's something it can hide) hides them in one flip — it starts checked, so nothing hides until you say so. Hiding takes three things together: the dark single-home dots; any **apartment building whose units are all restricted or no-soliciting** (a building with even one workable unit stays, whole); and the **shape and label of any book that's left with nothing on the map** — so a fully restricted book stops sitting there as a solid empty shape. A book with any live door keeps its shape, and if you've selected the **Restricted** pill the shapes stay too (you asked to see those books). If hiding empties the map of books entirely, a small note on the map says so. The bar across the top keeps counting the hidden homes and the book list on the left never hides anything, so no number changes. One thing to know: a hidden home can't be picked with **Select doors** — to bulk-unmark homes you've hidden, re-check the box first.

**Fully restricted buildings read differently.** An apartment building whose units are *all* restricted or no-soliciting paints slate (pink if it's all no-soliciting) instead of its book color, and its little badge says `12 restricted` (or `12 no soliciting`) rather than `12/12 hit` — every unit has an entry, but nobody actually got in. A building with a mix of statuses keeps its book color and the usual `5/12 hit` count.

**Hiding doors that aren't in a book.** After a cut — especially a second pass where you only cut the unknocked doors — every door left out of a book (the already-worked ones, restricted homes, voters added since you cut) still shows on the map as a gray dot, so the round can look bigger than the actual walk. The **Not in a book** checkbox in the **Layers** box hides them all in one flip. It starts hidden, so the map opens showing just the cut's books, and only appears when the cut left doors loose. Homes that are *inside* a book — including one a canvasser marked restricted mid-round — keep their book color and stay put, so you never lose worked doors; the one exception is yours to flip — the **Restricted & no soliciting** checkbox above.
