...
**Hiding restricted and no-soliciting dots.** Once status colors are on, restricted and no-soliciting single-family homes draw as the darkest dots on the map, and a round with a lot of them can bury the live work. A **Restricted & no soliciting** checkbox in the **Layers** box (it appears whenever any of those dots are on the map) hides them in one flip — it starts checked, so nothing hides until you say so. Apartment buildings keep all their units either way, and the bar across the top keeps counting the hidden homes, so no number changes. One thing to know: a hidden dot can't be picked with **Select doors** — to bulk-unmark homes you've hidden, re-check the box first.
